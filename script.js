...
if ((new URLSearchParams(window.location.search).get("q") || "").startsWith("!")) {
	...
	_$.getHTML(`https://cors.explosionscratc.repl.co/www..com/search?q=${escape((new URLSearchParams(window.location.search)).get("q").replace(/^!/, ""))}&btnI=Im+Feeling+Lucky`, () => {}).then(html => {
...
function run() {
	...
	if (inp.value === (new URLSearchParams(window.location.search)).get("q")) {
		...
		ftch(`httpsapis.explosionscratc.repl.co/google?q=${escape(inp.value)}`)
	}
	...
	window.onpopstate = (e) => {
		...
		ftch(`hapis.explosionscratc.repl.co/google?q=${escape(inp.value)}`)
	}
	inp.addEventListener("keyup", (e) => {
		if (key(e) && e.key !== "Enter") return;
		if ((e.key === "Enter" && e.shiftKey)) {
			console.log("Shift + enter was pressed, fetching first search result to redirect to...")
			_$.getHTML(`httpcors.explosionscratc.repl.co/wsearch?q=${escape(inp.value.replace(/^!/, ""))}&btnI=Im+Feeling+Lucky`, () => {}).then(html => {
		...
		if (e.key === "Enter") {
			...
			ftch(`https://apis.explosio?q=${escape(inp.value)}`);
		}
	...
	function ftch(url) {
		(async () => {
			...
			if (mode() == "images") {
				...
				width = 0;
				if (cache[`https://apis.explosionscratc.repl.co/im?q=${escape(inp.value)}`.toLowerCase()]) {
